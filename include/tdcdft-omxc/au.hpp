...
struct Effau {
    ...
    const double a0 = BOHR_in_Angstrom / (meff * eeff * eeff);          // effective Bohr radius in Angstrom
    const double a0cm = a0 * 1.e-8;                                     // effective Bohr radius in cm
    const double Eh = 1.e3 * HARTREE_in_eV * meff * std::pow(eeff, 4);  // effective Hartree energy in meV
    const double Eau =
        1.e-3 * Eh / (a0 * 1.e-10);    // V/m, effective atomic unit of electric field by Eh/e0a0 (note e not eeff!)
    const double EmVnm = 1.e-6 * Eau;  // mV/nm
    const double utfs = 1.e15 * AUOT_in_sec * HARTREE_in_eV / (Eh * 1.e-3);  // unit time in fs
