...
/*
        ...
        [1] C. A. Ullrich and Z.-H. Yang, A Brief Compendium of Time-Dependent Density Functional Theory, Braz. J. Phys.
   44, 154 (2014).

        Correlation:
                spin-unpolarized case in Chachiyo-Karasiev parametrization
        [2] T. Chachiyo, “Communication: Simple and accurate uniform electron gas correlation energy for the full range
   of densities”, J. Chem. Phys., vol. 	145, p. 021101, 2016. [3] V. V. Karasiev, “Comment on ‘Communication: Simple and
   accurate uniform electron gas correlation energy for the full range of densities”, J. Chem. Phys., vol. 145, p.
   157101, 2016.
*/
...
// exchange energy
vec get_ex(vec cr) { return ax * cr; }
...
vec get_n23f0(vec cr) {
    ...
    for (uword m = 0; m < cr.n_elem; ++m) {
        ...
        double cr3 = cr1 * cr2;
        double arg = 1. + B1 * cr1 + B2 * cr2;
        result(m) += ac * (4. * B1 + (3. * B1 * B1 + 10. * B2) * cr1 + 10. * B1 * B2 * cr2 + 6. * B2 * B2 * cr3) /
                     (9. * arg * arg);
    }
    return result;
}
...
mat get_n23f0finf(vec cr) {
    ...
    for (uword m = 0; m < cr.n_elem; ++m) {
        ...
        double arg = 1. + B1 * cr1 + B2 * cr2;
        // fxc.crho(m) = cr1;
        n23f0finf(m, 0) += ac * (4. * B1 + (3. * B1 * B1 + 10. * B2) * cr1 + 10. * B1 * B2 * cr2 + 6. * B2 * B2 * cr3) /
                           (9. * arg * arg);                                                           // f0
