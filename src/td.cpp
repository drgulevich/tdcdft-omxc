...
mat to_ksbasis(Mesh<QuantumWell> &mesh, KsGs &ks, vec &v) {
    ...
    for (uword n1 = 0; n1 < dim; ++n1)
        for (uword n2 = 0; n2 < dim; ++n2) vmatrix(n1, n2) = mesh.dz * sum(ks.orbs.col(n1) % v % ks.orbs.col(n2));
    return vmatrix;
}
...
cx_mat grad(Mesh<QuantumWell> &mesh, cx_mat &psi) {
    ...
    for (uword m = 0; m < psi.n_cols; ++m) {
        result(span(1, dim - 2), m) =
            factor * (psi.col(m).tail(dim - 2) - psi.col(m).head(dim - 2));      // central differences
        result(0, m) = factor * (-3. * psi(0, m) + 4. * psi(1, m) - psi(2, m));  // 2nd order edge
        result(dim - 1, m) =
            -factor * (-3. * psi(dim - 1, m) + 4. * psi(dim - 2, m) - psi(dim - 3, m));  // 2nd order edge
    }
    return result;
}
...
cx_mat get_memory(cx_mat &p, cx_mat &previous_memory, vec &previous_gradv, vec &gradv, double dt) {
    ...
    for (uword ncol = 0; ncol < ncols; ++ncol)
        for (uword m = 0; m < nrows; ++m) {
            ...
            //			memory(m,ncol) = previous_memory(m,ncol)-i*p(m,ncol)*dt*previous_memory(m,ncol) +
            // dt*previous_gradv(m); // same results as EXP_ORDER=1 			memory(m,ncol) =
            // previous_memory(m,ncol)-i*p(m,ncol)*dt*previous_memory(m,ncol) + dt*gradv(m); // more stable but
            // overdamped
        }
...
/**
 * Get the memory part of a scalar Vxc potential within the ALDA+M theory (only in quasi-1D with one nontrivial
 * direction)
 */
mat get_VxcM(Mesh<QuantumWell> &mesh, cx_mat &memory, vec &n13, cx_mat &n23C) {
    vec n23sxc = real(n23C.col(0) % memory.col(0));
    for (uword m = 1; m < n23C.n_cols; ++m) n23sxc += real(n23C.col(m) % memory.col(m));
 ...
 */
double get_dipole(Mesh<QuantumWell> &mesh, vec &rho) { return mesh.dz * sum(mesh.z % rho); }
...
TdDipole Tdks(QuantumWell &qwell, Mesh<QuantumWell> &mesh, xc::Omxc &fxc, KsGs &ks, TdArgs args) {
    ...
    for (uword m = 0; m < qwell.Nocc; ++m) weights(m) = (qwell.EF - ks.ens(m)) / M_PI;
    ...
    do {
        ...
        for (int nc = 0; nc < args.ncorrections; ++nc) {
            ...
            for (uword m = 0; m < qwell.Nocc; ++m) rho += weights(m) * real(conj(psi.col(m)) % psi.col(m));
            ...
            if (fxc.Mosc >= 0) {  // ALDA and beyond
                ...
                if (fxc.Mosc > 0) {  // nonadiabatic
                    ...
                    // Calculate current
                    vec J = zeros(mesh.M);
                    for (uword m = 0; m < qwell.Nocc; ++m) J += weights(m) * imag(conj(psi.col(m)) % gradpsi.col(m));
                    vec vfield = J / rho;
