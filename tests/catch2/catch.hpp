...
#define INTERNAL_CATCH_TYPE_GEN                                                                                       \
    ...
    template <template <typename...> class Container, template <typename...> class List, class... Elems,              \
              typename... Elements>                                                                                   \
    constexpr auto rewrap(List<Elems...>, Elements...) noexcept->decltype(append(TypeList<Container<Elems...>>{},     \
    ...
    constexpr auto create(                                                                                            \
        TypeList<Types...>) noexcept->decltype(append(Final<>{}, rewrap<Containers>(Types{}...)...)) {                \
        return {};                                                                                                    \
    }                                                                                                                 \
...
#define INTERNAL_CATCH_NTTP_1(signature, ...)                                                                         \
    ...
    template <template <INTERNAL_CATCH_REMOVE_PARENS(signature)> class Container,                                     \
              template <INTERNAL_CATCH_REMOVE_PARENS(signature)> class List, INTERNAL_CATCH_REMOVE_PARENS(signature), \
              typename... Elements>                                                                                   \
    constexpr auto rewrap(List<__VA_ARGS__>, Elements... elems) noexcept->decltype(append(                            \
...
namespace Catch {
void writeToDebugConsole(std::string const& text) { __android_log_print(ANDROID_LOG_DEBUG, "Catch", text.c_str()); }
}  // namespace Catch
...
std::string WithinUlpsMatcher::describe() const {
    ...
    return ret.str();
    // return "is within " + Catch::to_string(m_ulps) + " ULPs of " + ::Catch::Detail::stringify(m_target) + ((m_type ==
